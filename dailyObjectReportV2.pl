...
sub printHeader {
  ...
  printf "<TR BGCOLOR=lightgreen><TD>Client</TD><TD>Cluster</TD><TD>Status</TD><TD>Level</TD><TD>Size</TD><TD>Started</TD><TD>Duration</TD><TD>Expires</TD></TR>" if ($display==1);
}
  ...
  foreach my $href (@clusters){
    ...
    while(my @rows=$sth->fetchrow_array){
      ...
      if($rows[2] eq "Failure"){
        push(@failure, "$rows[0],$rows[1],$rows[2],$rows[3],$rows[4],$rows[5],$rows[6],$rows[7]");
      } elsif($rows[2] eq "Success"){
        push(@success, "$rows[0],$rows[1],$rows[2],$rows[3],$rows[4],$rows[5],$rows[6],$rows[7]");
      } else {
        push(@warning, "$rows[0],$rows[1],$rows[2],$rows[3],$rows[4],$rows[5],$rows[6],$rows[7]");
      }
    }
    $dbh->disconnect();
  }
...
sub printReport {
  ...
  foreach(@data){
    ...
    printf "<TR bgcolor=".$bgColor." style=color:".$textColor."><TD>$cols[0]</TD><TD>$cols[1]</TD><TD>$cols[2]</TD><TD>$cols[3]</TD><TD>$cols[4]</TD><TD>$startTime</TD><TD>%d Hours %d Minutes %d Seconds</TD><TD>$expireTime</TD></TR>\n",$hours,$minutes,$seconds if ($display==1);
  }
  printf "</TABLE><br/>\n" if ($display==1);
  printf "</BODY></HTML>\n" if ($display==1);
}  
